...
- [~] **Split cmd/inber/cli_test.go** — 564 lines containing multiple unrelated test categories (repo operations, agents, models, sessions, memory, config, text processing). Extract each category into focused test files: `cli_repo_test.go`, `cli_agents_test.go`, `cli_models_test.go`, `cli_sessions_test.go`, `cli_memory_test.go`, `cli_config_test.go`, `cli_text_test.go`. Keep shared test helpers in `cli_test.go`. Better separation of concerns and easier maintenance.
