...
func NewEngineBenchmark(ctx context.Context, cfg EngineConfig) (*Engine, BenchmarkTiming, error) {
	...
	phaseStart = time.Now()
	agentRegistry, err := setupAgentRegistry(e.AgentConfig, cfg.ExtraTools, e.Client, repoRoot, modelClient, e.modelStore, e.MemStore, cfg.AgentStorePath, cfg.LogstackURL, cfg.InberServerURL, cfg.ToolConnections)
	if err != nil {
		return nil, timing, err
	}
