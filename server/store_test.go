...
func TestStoreSessionCRUD(t *testing.T) {
	...
	// Create session.
	if err := s.UpsertSession("agent:claxon:main", "claxon", "main", SessionLineage{}, nil); err != nil {
		t.Fatal(err)
	}
	...
	s.UpsertSession("agent:claxon:main", "claxon", "main", SessionLineage{}, nil)
	...
	// Filter by kind.
	s.UpsertSession("agent:ogma:spawn-123", "ogma", "spawn", SessionLineage{}, nil)
...
func TestStoreSpawnChain(t *testing.T) {
	s := tempStore(t)
	s.UpsertSession("agent:claxon:main", "claxon", "main", SessionLineage{}, nil)
	s.UpsertSession("agent:ogma:spawn-1", "ogma", "spawn", SessionLineage{}, nil)
