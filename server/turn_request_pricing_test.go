...
//
// That is deliberate. Every price on this host's real registry is uniformly
// cheaper or uniformly dearer than the fallback, so a cost computed from the
// fallback is the registered cost times a single constant, and an assertion can
// be satisfied by a calculation that scaled rather than looked up. Straddling
// removes that: no single factor carries $5.00/$2.00 to $3.00/$15.00.
const (
