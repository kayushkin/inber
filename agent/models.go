...
// request can carry without the long-context beta header. newAnthropicClient
// (clients.go) sets the anthropic-beta header, listing every beta inber sends,
// and the long-context one is not among them, so a registry row
