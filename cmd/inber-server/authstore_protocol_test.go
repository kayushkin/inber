...
//
// The function takes auth-store's URL and token as arguments and returns the
// secret; it reads and writes no environment variable.
...
func TestResolveUsesTheRouteAuthStoreServes(t *testing.T) {
	...
	if _, err := resolveAnthropicFromAuthStore(context.Background(), "inber-server", got.url, "probe-token"); err != nil {
...
func TestTheAppArgumentReachesAuthStoreAsTheIntendedApp(t *testing.T) {
	...
	if _, err := resolveAnthropicFromAuthStore(context.Background(), "inber-bench", got.url, "probe-token"); err != nil {
		t.Fatalf("resolveAnthropicFromAuthStore: %v", err)
	}
...
func TestResolveReturnsTheSecret(t *testing.T) {
	got := newRecordingAuthStore(t, 200, resolveFixtureFromRealAuthStore)

	secret, err := resolveAnthropicFromAuthStore(context.Background(), "inber-server", got.url, "probe-token")
	if err != nil {
		t.Fatalf("resolveAnthropicFromAuthStore: %v", err)
	}
...
func TestResolveAcceptsAnOAuthCredentialAsAnAccessToken(t *testing.T) {
	...
	got := newRecordingAuthStore(t, 200, `{"auth_type":"oauth","access_token":"tok-oauth"}`)
	...
	if secret != "tok-oauth" {
		t.Errorf("secret = %q, want the access_token", secret)
	}
}

// TestResolveFailsLoud pins the single-source-of-truth contract in the doc
// comment: on any failure the caller gets an error and no secret, so it has
...
func TestResolveFailsLoud(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := newRecordingAuthStore(t, tc.status, tc.body)
			...
			if secret != "" {
				t.Errorf("secret = %q; a failed resolve must return none", secret)
			}
		})
	}
}
...
func TestResolveRefusesToRunWithoutTheConfigurationItNeeds(t *testing.T) {
	t.Run("empty app", func(t *testing.T) {
		got := newRecordingAuthStore(t, 200, resolveFixtureFromRealAuthStore)
		if _, err := resolveAnthropicFromAuthStore(context.Background(), "", got.url, "probe-token"); err == nil {
			t.Fatal("an empty app was accepted; it becomes X-Auth-App, which auth-store requires")
		}
	})
	t.Run("missing token", func(t *testing.T) {
		got := newRecordingAuthStore(t, 200, resolveFixtureFromRealAuthStore)
		_, err := resolveAnthropicFromAuthStore(context.Background(), "inber-server", got.url, "")
		if err == nil {
