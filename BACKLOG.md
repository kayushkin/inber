...
- [~] **Extract tool assembly from server/workspace_tools.go** — The `toolsForAgent()` function (314-line file) mixes workspace management tools with references to spawn tools from another file. Extract tool assembly logic into `server/agent_tools.go` to centralize tool composition. Keep workspace-specific tools (merge, reject, fix, list) in `workspace_tools.go`. Better separation of concerns between tool assembly and workspace operations.
