...
- [~] **Split session/prompts.go** — 334 lines combining file writing, token estimation, and file reading. Extract token estimation into `prompts_tokens.go`, file writing into `prompts_write.go`, and file reading/listing into `prompts_read.go`. Keep core types and utilities in main file. Better separation of concerns: prompt breakdown writing vs token calculation vs reading operations.
