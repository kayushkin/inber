...
// pruneToolResult applies age-based pruning to tool result content
func pruneToolResult(block anthropic.ContentBlockParamUnion, age int, cfg PruneConfig) (anthropic.ContentBlockParamUnion, bool) {
	...
	// Very recent: keep full
	if age <= cfg.ToolResultKeepFull {
		return block, false
	}

	// Middle age: summarize
	if age <= cfg.ToolResultSummary {
...
// truncateToolCall simplifies old tool call inputs
func truncateToolCall(block anthropic.ContentBlockParamUnion) anthropic.ContentBlockParamUnion {
	...
	// Create a summary of the input instead of full parameters
	summary := fmt.Sprintf("called %s", toolUse.Name)
	if inputMap, ok := toolUse.Input.(map[string]interface{}); ok {
		if len(inputMap) > 0 {
			summary += fmt.Sprintf(" with %d parameters", len(inputMap))
		}
	}
