...
- [~] **Split agent/registry/registry_test.go** — 455-line monolithic test file combining mock setup, tool registry tests, registry creation tests, and registry core functionality tests. Extract into logical groups: `registry_test_mocks.go` (mock implementations and helpers), `tool_registry_test.go` (tool registration and memory/spawn tools), `registry_creation_test.go` (registry creation and factory functions), and `registry_core_test.go` (agent listing, configuration, session management). Better separation of concerns and easier test maintenance.
