...
- [ ] **Agent Zero** (github.com/agent0ai/agent-zero) — Python. Dynamic, self-extending agent framework. No predefined tools — agent writes its own code/scripts as needed. Persistent memory, multi-agent cooperation, Docker isolation. Web UI. Study their "organically growing" approach and how agent-created tools compare to inber's static tool registry.
- [ ] **Commander** (github.com/autohandai/commander) — Tauri v2 desktop app (Rust + React/Vite). Orchestrates multiple CLI coding agents (Claude Code, Codex, Gemini) against Git projects. Git worktree-based isolation per agent. Study their multi-agent orchestration UI pattern and worktree management — similar to inber's forge workspace system.
- [ ] **Autohand Code CLI** (github.com/autohandai/code-cli) — TypeScript/Bun. Autonomous coding agent CLI using ReAct pattern. Multi-provider (OpenRouter, Anthropic, OpenAI, local). Modular skill system, git integration. Study their ReAct implementation and skill extensibility model.
- [ ] **Cline** (github.com/cline/cline) — VS Code extension. Autonomous coding agent in-editor with human-in-the-loop approval for every action. File ops, terminal commands, browser automation, MCP support. Study their approval UX, AST-based codebase analysis, and how they manage context for large projects.
- [ ] **Roo Code** (github.com/RooCodeInc/Roo-Code) — VS Code extension (Cline fork). Multi-mode agents (Code, Architect, Ask, Debug, custom). MCP server support, slash-command skills. Study their mode system as a pattern for inber's agent specialization — different system prompts/tools per mode rather than per agent.
...
- [ ] **Forge (jdforsythe)** (github.com/jdforsythe/forge) — Claude Code plugin. Science-backed AI team assembly using DeepMind research, PRISM persona science, and vocabulary routing. Decomposes goals into agent teams with optimal topology (3-5 agents). Skills: mission-planner, agent-creator, skill-creator, librarian. Study their vocabulary routing concept (domain terminology activating expert knowledge) and structured artifact handoff patterns.
- [ ] **Jig** (github.com/jdforsythe/jig) — Go. Profile manager and launcher for Claude Code sessions. Define tools, MCP servers, skills, agents, permissions per profile, launch with one keystroke. TUI profile picker, profile inheritance, ad-hoc `--pick` mode. Study their profile resolution/inheritance model and how they compose Claude Code sessions from reusable components.
