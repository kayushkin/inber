...
- [x] **Herdr** (herdr.org) — Orchestration-type meta-harness (control layer *above* harnesses, same category as inber's orchestration half — cf. Omnigent). Hosted/closed, no public source: routes tasks to model/tool/specialist agent by **context/cost/policy/urgency**, shared fleet "workflow memory" (prior decisions, system rules, human feedback), non-blocking human gates, fleet observability (trace decisions, inspect handoffs, throughput). Takeaway is framing not blueprint — the one thing inber under-develops is **scored task→agent routing** (inber picks the *model* per turn but the *agent* by board/availability). See `docs/comparisons/herdr.md`. **Distinct namesake:** herdr.dev (github.com/ogulcancelik/herdr) is a Rust TUI "tmux-for-agents" multiplexer (ratatui/tokio, 14+ agent integrations, AGPL) — groups with commander.md / inber-party, not here. Do not conflate.
