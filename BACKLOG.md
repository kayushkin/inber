...
- [~] **Split cmd/inber/sessions.go** — 391-line file combining 7 different session-related commands: list, show, context, prompts, prompt, timeline, and active. Extract logical groups: `sessions_listing.go` (list and show commands), `sessions_analysis.go` (context, prompts, prompt commands), `sessions_timeline.go` (timeline command), and `sessions_active.go` (active sessions command). Keep command setup and shared utilities in main file. Better separation of concerns and easier maintenance.
