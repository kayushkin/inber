...
- [~] **Split conversation/manage_utils.go** — Extract 341-line utility file into focused modules: `manage_tool_pruning.go` (tool result/call pruning and summarization), `manage_auto_save.go` (auto-save to memory functionality), and `manage_text_utils.go` (text processing and content extraction utilities). Better separation of concerns between tool management, memory operations, and text processing.
