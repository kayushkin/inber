...
- [~] **Split engine/display.go** — 384 lines combining color constants, logger functionality, tool display, content display, and stats display. Extract into focused modules: `display_colors.go` (ANSI constants), `display_logger.go` (Logger struct + methods), `display_tools.go` (tool call/result formatting), `display_content.go` (thinking/response display), `display_stats.go` (turn/session statistics). Better separation of concerns.
