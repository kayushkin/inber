...
- [~] **Split session/timeline.go** — Extract cost calculation functions into `timeline_cost.go`, formatting functions into `timeline_format.go`, JSONL operations into `timeline_jsonl.go`, and utility functions into `timeline_utils.go`. Keep core types and main timeline logic in `timeline.go`. Better separation of concerns: cost calculation, formatting, file I/O, and utilities are distinct responsibilities. Currently 394 lines combining 5 different concerns.
