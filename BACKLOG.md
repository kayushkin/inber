...
- [~] **Split server/session.go** — Currently 398 lines combining multiple distinct concerns: session lifecycle (setOnEvent, updateHooks, turn, inject, stop, close), session creation/management (getOrCreateSession, createSession, loadPersistedMessages), session forking (forkSession, sessionKeyForChild), context injection (contextInjectorsFor, sessionStatusInjector), and utility functions (formatDuration, truncate). Extract session creation into `session_creation.go`, forking into `session_forking.go`, context injection into `session_context.go`, and utilities into `session_utils.go`. Keep core Session struct and lifecycle in main file. Better separation of concerns.
