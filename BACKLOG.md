...
- [x] **NanoClaw** (github.com/qwibitai/nanoclaw) — Lightweight OpenClaw alternative. Built on Anthropic's Agent SDK / Claude Code. Agents run in Linux containers (Docker or Apple Container) for OS-level isolation. Small codebase, "fork and customize" model. Skills-based extension via Claude Code commands. Study their container isolation approach vs inber's workspace confinement, and the tradeoff of tiny-codebase-you-modify vs framework-with-config.
- [x] **Agent Zero** (github.com/agent0ai/agent-zero) — Python. Dynamic, self-extending agent framework. No predefined tools — agent writes its own code/scripts as needed. Persistent memory, multi-agent cooperation, Docker isolation. Web UI. Study their "organically growing" approach and how agent-created tools compare to inber's static tool registry.
- [x] **Commander** (github.com/autohandai/commander) — Tauri v2 desktop app (Rust + React/Vite). Orchestrates multiple CLI coding agents (Claude Code, Codex, Gemini) against Git projects. Git worktree-based isolation per agent. Study their multi-agent orchestration UI pattern and worktree management — similar to inber's forge workspace system.
- [x] **Meta-Harness** (arXiv:2603.28052) — Stanford/MIT paper. Automated search over harness code using a coding agent proposer with full filesystem access. Validates that harness engineering matters enormously. Study their filesystem-as-memory pattern and how they optimize prompt/retrieval/orchestration code.
- [x] **Autohand Code CLI** (github.com/autohandai/code-cli) — TypeScript/Bun. Autonomous coding agent CLI using ReAct pattern. Multi-provider (OpenRouter, Anthropic, OpenAI, local). Modular skill system, git integration. Study their ReAct implementation and skill extensibility model.
- [x] **Cline** (github.com/cline/cline) — VS Code extension. Autonomous coding agent in-editor with human-in-the-loop approval for every action. File ops, terminal commands, browser automation, MCP support. Study their approval UX, AST-based codebase analysis, and how they manage context for large projects.
- [x] **Roo Code** (github.com/RooCodeInc/Roo-Code) — VS Code extension (Cline fork). Multi-mode agents (Code, Architect, Ask, Debug, custom). MCP server support, slash-command skills. Study their mode system as a pattern for inber's agent specialization — different system prompts/tools per mode rather than per agent.
- [x] **Dexto** (github.com/truffle-ai/dexto) — TypeScript. Agent harness / orchestration layer. Config-driven agents via YAML, session management, tool orchestration, memory, multi-agent systems. Ships with coding agent. Runs as CLI, Web UI, REST API, Discord, Telegram. Study their YAML agent definition pattern and "OS for agents" abstraction vs inber's Go-native approach.
- [x] **Forge (jdforsythe)** (github.com/jdforsythe/forge) — Claude Code plugin. Science-backed AI team assembly using DeepMind research, PRISM persona science, and vocabulary routing. Decomposes goals into agent teams with optimal topology (3-5 agents). Skills: mission-planner, agent-creator, skill-creator, librarian. Study their vocabulary routing concept (domain terminology activating expert knowledge) and structured artifact handoff patterns.
- [x] **Jig** (github.com/jdforsythe/jig) — Go. Profile manager and launcher for Claude Code sessions. Define tools, MCP servers, skills, agents, permissions per profile, launch with one keystroke. TUI profile picker, profile inheritance, ad-hoc `--pick` mode. Study their profile resolution/inheritance model and how they compose Claude Code sessions from reusable components.
- [x] **Bub** (github.com/bubbuild/bub) — Python. Hook-first agent runtime built on `pluggy` (pytest's plugin system). Every turn stage (`resolve_session → load_state → build_prompt → run_model → save_state → render_outbound → dispatch_outbound`) is a hook; builtins use the same surface as third-party plugins. Append-only "tape context" rebuilt per turn instead of mutable session state. One pipeline drives CLI, Telegram, and custom channels. Built on agents.md + Agent Skills standards. Originated in group chats requiring multi-actor shared context. Study their pluggy-based pipeline as a model for making inber's turn loop hook-driven, and the append-only tape as an alternative to mutating `Conversation`.
- [x] **llm-gateway / RLM harness** (github.com/hvent90/llm-gateway, packages/ai/rlm) — TypeScript. Composable agent framework where every component (provider, agent loop, RLM, orchestrator) implements `GeneratorHarnessModule` and yields a typed event stream — wrap freely. The **RLM (Recursive Language Model) harness** is the standout: instead of stuffing long inputs into context, input is bound to a REPL variable named `context`; the model only sees metadata (length, prefix) and writes JS to query it via `FINAL()`/`exec()`/`llm_query()`. HITL gating via async-queue relay events. Study the streaming-harness composition primitive as a model for inber's engine, and pilot an RLM-style harness over memory as an alternative to smart-truncation.
- [x] **Herdr** (herdr.dev, github.com/ogulcancelik/herdr) — Rust, Apache-2.0. An **agent multiplexer** ("tmux for coding agents"): runs foreign agent CLIs in PTY panes, *infers* each agent's lifecycle state (`idle/working/blocked/done/unknown`) from the outside, and exposes a Unix-socket JSON-RPC API so agents script each other. Studied because it has production answers to three harness-bridge problems inber faces from wrapping foreign harnesses: **(1)** two-channel state inference — a versioned per-agent TOML rule engine scraping named screen regions **plus** an injected lifecycle hook — the reusable design behind inber's `awaiting_user`/herald `ask` for PTY-mode harnesses that emit no stream-json; **(2) live server handoff** (`handoff_runtime.rs` passes PTY master fds + session state to a replacement daemon binary) — directly attacks the "never restart the gateway unattended" landmine; **(3)** server-owned `agent.wait` — the missing multi-agent coordination verb. Reliability work (serialize lifecycle reports, settle events, stall-timeout, overlay-suppress) is as valuable as the ideas. Native session-id capture + resume echoes inber's harness-session-id contract. See `docs/comparisons/herdr.md`. **Namesake trap:** herdr.org is an unrelated closed marketing "orchestration" page with no public substance — ignore it.
- [x] **Marionette** (github.com/professorpalmer/marionette, formerly `pm-harness`) — Python stdlib backend + React/Electron desktop app, ~89k lines of `harness/` against 4,204 test functions. A **pilot shell over a separate durable-state kernel**: it calls the same author's Puppetmaster (`puppetmaster-ai`, MIT) in-process via `Orchestrator(store).run(...)` and holds a **read-only** view of job state, so orchestration is not a tool the model calls. Studied because it has working versions of three things inber has only the *shape* of: **(1)** `command_policy.py` classifies the **command string**, not the tool name — it labels and never rewrites, and only bites in full-auto — against inber's single `isDangerous("shell_commands")` bit, which permission-store's finished AST splitter could fix; **(2)** `autobudget.py` adds `max_idle_steps=3` and a killswitch stop-file to the usual token/time caps, and links a child's ceiling to its parent's — the reader inber's unwired `RecordToolCall`/`IsRepeating` pair has been missing, plus the out-of-band abort a wedged engine needs; **(3)** `prompt_cache.py` holds cache-control as a **per-provider table** (explicit for Anthropic/Qwen via OpenRouter, automatic for OpenAI/Gemini/DeepSeek/Grok) with strip-and-retry on 400 — the corrective to inber's `__CACHE_BOUNDARY__`, deleted 2026-08-07 as a sentinel no block ever carried. Also: `codegraph_preflight.py` recommends excludes *before* indexing (inber's `codeindex` is 88 lines of `// TODO`), and swarm workers get a distilled goal brief, never the transcript. See `docs/comparisons/marionette.md`. ⚠️ **No LICENSE file anywhere in the tree — all-rights-reserved; learn from it, do not copy code.** ⚠️ **Both headline benchmark numbers overstate their own sources**: the README's "47% cheaper at equal quality" is "47–48% token-matched savings… does not establish quality parity" in the same author's Puppetmaster README, and the NL2Repo-Bench 91.1% drops the paper's own "not a head-to-head model comparison" caveat. The in-repo `bench/cache_wars.py` is the opposite and worth imitating — it ships a **"Claim hygiene"** section naming what its 66.1% figure does *not* prove.
...
- [x] **Consolidate small session utility files** — Consolidated micro-files to reduce file fragmentation: moved `cost()` and `calculateTurnCost()` from `session/cost.go` (17 lines) into `session/session.go` (_`calculateTurnCost` was deleted later, in `961f6dd`: it held a private copy of the input/output arithmetic with no cache terms, and the session now prices through `session.CalcCostWithCache`_), and moved `Close()` method and `nullStr()` utility from `session/db.go` (18 lines) into `session/db_types.go` where they logically belong. Removed 35 lines of fragmented files while maintaining clean organization. All builds and tests pass.
...
- [x] **Split conversation/extract.go** — Extracted types/configuration into `extract_types.go` (42 lines), prompt constants into `extract_prompts.go` (33 lines), and utility functions into `extract_utils.go` (39 lines). Reduced main extract.go from 297 to 184 lines (113-line/38% reduction). Better separation of concerns: type definitions vs prompts vs utilities vs core extraction logic. All conversation tests pass.

- [x] **Split agent/registry/registry_test.go** — Extracted 455-line monolithic test file into logical groups: `registry_test_mocks.go` (116 lines: mock implementations and test helpers), `tool_registry_test.go` (94 lines: tool registration and memory/spawn tools tests), `registry_creation_test.go` (47 lines: registry creation and factory function tests), and `registry_core_test.go` (149 lines: agent listing, configuration, and session management tests). Reduced main registry_test.go from 455 to 7 lines (98% reduction). Better separation of concerns and easier test maintenance. All tests passing.
